...
/* With the introduction some smart pointers, there is no need to handle destruction implicit here for the nodes.
Also, with the chnages of ownership to the edges, this logic is also removed from here.*/
ChatLogic::~ChatLogic()
{
}
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    // Task 5
    ChatBot scopedBot("../images/chatbot.png");

    SetChatbotHandle(&scopedBot);

    std::cout << "Executing task 5 2" << std::endl;

    scopedBot.SetChatLogicHandle(this);

    std::cout << "Executing task 5 3" << std::endl;
    scopedBot.SetRootNode(rootNode);
    //_chatBot = &scopedBot;

    std::cout << "Executing task 5 4" << std::endl;
    rootNode->MoveChatbotHere(std::move(scopedBot));

    std::cout << "Executing task 5 5" << std::endl;
}
